  ...
  return `
    <div class="animal-card">
      ...
      <div class="animal-card-body">
        ...
        <div class="animal-card-meta">${escHtml(a.species)}${a.breed ? ' · ' + escHtml(a.breed) : ''} · Age: ${a.age < 1 ? Math.round(a.age * 12) + ' mos' : a.age + ' yr/old'}</div>        <div class="status-badge status-${statusClass}">${a.status || 'Available'}</div>
...
function openAnimalModal(id) {
  ...
  modalContent.innerHTML = `
    ...
    <div class="modal-meta">${escHtml(a.species)} · ${escHtml(a.breed || 'Mixed')} · Age: ${a.age < 1 ? Math.round(a.age * 12) + ' mos' : a.age + ' yr/old'} · <span class="status-badge status-${(a.status||'available').toLowerCase()}">${a.status}</span></div>
